...
class tfCMF:
    def __init__(self, alpha=0.7, d_hidden=5, lamda=1, left=lambda x: x ** 2 / 2, right=tf.math.softplus):
        ...
        exp is link of poisson loss. (tf.math.exp)
    ...
    def train(self, X_train, X_test, Y, mask_train=None, mask_test=None, optim_steps=20000, verbose=100, early_stopping=5, lr=0.005, optimizer="Adam"):
        """[summary]

        Args:
            X_train (matrix): main matrix for train. The shape is (n × m).
            X_test (matrix): main matrix for test. The shape is (n × m).
            ...
            verbose (int, optional): num of step for logs are displayed. Defaults to 100.
        ...
        U, V, Z = self.init_matrices(X_train, Y)
        ...
        X_test = tf.keras.backend.constant(X_test, dtype=tf.float32)
        ...
        if mask_test is None:
            M_test = tf.keras.backend.constant(
                np.ones(X_test.shape).astype(int))
        elif mask_test == "nonzero":
            M_test = tf.keras.backend.constant((X_test != 0).astype(int))
        else:
        ...
        for times in tqdm(range(optim_steps)):
            ...
            loss_test = loss(X=X_test, Y=Y, M=M_test).numpy()
